  ...
  return (
    <>
      <div className="w-full flex items-center justify-between bg-white h-[5rem] min-h-[5rem] rounded-[10px] overflow-hidden p-0 shadow-custom-shadow">
        ...
        <div className="hidden lg:flex items-center justify-center gap-[4rem] w-[50%]">
          {navBarItem.map((item) => (
            <NavLink
              key={item.name}
              to={`${item.link}`}
              className={({ isActive }) =>
                isActive
                  ? "text-primary text-[1.2rem] hover:text-primary"
                  : "text-secondary hover:text-primary text-[1.2rem]"
              }
            >
              {item.name}
            </NavLink>
        ...
        <IoMdMenu
          className="lg:hidden text-3xl text-primary mr-3"
      ...
      {isMobile && <MobileSideBar onClick={() => setIsMobile(false)} />}
    </>
  );
};
